...
half4  ExtinctColor (half4 baseColor, half extinctionAmount) 
{
	...
	half3 GerstnerOffsets (half2 xyVtx, half steepness, half amp, half freq, half speed, half2 dir) 
	{
		half3 offsets;
		
		offsets.x =
			steepness * amp * dir.x *
			cos( freq * dot( dir, xyVtx ) + speed * _Time.x); 
		...
		offsets.y = 
			amp * sin ( freq * dot( dir, xyVtx ) + speed * _Time.x);

		return offsets;			
	}	

	half3 GerstnerOffset4 (half2 xyVtx, half4 steepness, half4 amp, half4 freq, half4 speed, half4 dirAB, half4 dirCD) 
	{
		half3 offsets;
		
		half4 AB = steepness.xxyy * amp.xxyy * dirAB.xyzw;
		half4 CD = steepness.yyww * amp.yyww * dirCD.xyzw;
		
		half4 dotABCD = freq.xyzw * half4(dot(dirAB.xy, xyVtx), dot(dirAB.zw, xyVtx), dot(dirCD.xy, xyVtx), dot(dirCD.zw, xyVtx));
		half4 TIME = _Time.yyyy * speed;
		
		half4 COS = cos (dotABCD + TIME);
		half4 SIN = sin (dotABCD + TIME);
		
		offsets.x = dot(COS, half4(AB.xy, CD.xy));
	...
	half3 GerstnerNormal (half2 xyVtx, half steepness, half amp, half freq, half speed, half2 dir) 
	{
	...
	half3 GerstnerNormal4 (half2 xyVtx, half4 amp, half4 freq, half4 speed, half4 dirAB, half4 dirCD) 
	{
		half3 nrml = half3(0,2.0,0);
		
		half4 AB = freq.xxyy * amp.xxyy * dirAB.xyzw;
		half4 CD = freq.yyww * amp.yyww * dirCD.xyzw;
		...
		nrml.x -= dot(COS, half4(AB.xy, CD.xy));
		nrml.z -= dot(COS, half4(AB.yw, CD.yw));
		
		nrml.xy *= _GerstnerIntensity;
		nrml = normalize (nrml);
	...
	{
		#ifdef WATER_VERTEX_DISPLACEMENT_ON
			offs = GerstnerOffset4(tileableVtx.xy, steepness, amplitude, frequency, speed, directionAB, directionCD);
			nrml = GerstnerNormal4(tileableVtx.xy + offs.xy, amplitude, frequency, speed, directionAB, directionCD);		
		#else
			offs = half3(0,0,0);
			nrml = half3(0,1,0);
		#endif							
